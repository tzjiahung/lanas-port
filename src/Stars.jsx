...
// Deterministic PRNG — stable star fields across re-renders.
...
function FourPointStar({ x, y, size, color, glow, opacity = 1, glowOn = false, twinkleOn = false, twinkleDelay = 0 }) {
  const r = size / 2
...
function buildVariant(warmth, lightness) {
  ...
  const palette = [
    ...
    { color: starTint(lerpHex('#c97aa0', '#d99989', k)), glow: `rgba(207,130,150,${0.18 * (1 - lk * 0.5)})` },
    { color: starTint(lerpHex('#c8869b', '#d4a892', k)), glow: `rgba(200,140,155,${0.16 * (1 - lk * 0.5)})` },
    { color: starTint(lerpHex('#bd8a9a', '#c9a896', k)), glow: `rgba(189,138,154,${0.14 * (1 - lk * 0.5)})` },
  ]
  return { stops, palette }
}

// Production defaults — tweaks panel stripped
const T = {
  warmth: 56,
  depth: 42,
  lightness: 50,
...
export default function PeachDawn({ w, h }) {
  const { stops, palette } = useMemo(() => buildVariant(T.warmth, T.lightness), [])
  const stars = useMemo(
    () => generateStars({ count: T.starDensity, w, h, seed: 7, palette, sizeMix: T.starSizeMix }),
    [w, h, palette]
  )

  const depthK = T.depth / 100
  const vignetteAlpha = 0.10 + depthK * 0.32
  const fadeK = T.bottomFade / 100
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} width="100%" height="100%" style={{ display: 'block' }}>
      ...
      {stars.map((s, i) => (
        <FourPointStar key={i} {...s} glowOn={T.glow} twinkleOn={T.twinkle} twinkleDelay={i} />
      ))}
